...
function searchSuggest2() {
 if (searchReq2.readyState == 4 || searchReq2.readyState == 0) {
  var str = escape(document.getElementById('phnum').value);
  if(str.length > 12){ //alert('In');
  searchReq2.open("GET", 'searchSuggest2.php?search=' + str, true);
...
function handlesearchSuggest2() {
 if (searchReq2.readyState == 4) {
  ...
  var str1 = document.getElementById('phnum');
  var curLeft=0;
  ...
  var str2 = document.getElementById('phnum');
  var curTop=20;
...
function setSearch2(value) {
 document.getElementById('phnum').value = value;
