...
//GET App types
    ...
    $qry_accname  = "SELECT * FROM  accounts where account_name='online user'";
        if($db->query($qry_accname) && $db->get_num_rows() > 0){
            $vehiclepref = $db->fetch_all_assoc();
            // print_r($vehiclepref);
            $name = $vehiclepref[0]['account_name'];
            $vid = $vehiclepref[0]['id'];
        } 

        ///****************Counter Count the Trips for account of Online User **************//
        
    $qry_accname  = "SELECT count(account) as counter FROM  request_info where account=9";
        if($db->query($qry_accname) && $db->get_num_rows() > 0){
            // print_r( $db->fetch_one_assoc()['counter']);
            $Counter = $db->fetch_one_assoc()['counter'];
        } 
        // print_r($Counter);
        // exit;
    if($name = "online user"||$name = "Online User"||$name="ONLINE USER"){
    if(isset($_POST['submit'])){ 
        ...
        if($destination_lastX[1]){	$destination_last =	$destination_lastX[0].','.str_replace(',','',sql_replace($_POST['bsuiteroom'])).','.$destination_lastX[1]; }
            ...
            echo "<script type=javascript>alert('Only Online User Can Submit It')</script>";
        }
        ...
        //   print_r($address.'/request-a-trip.html');
          $db->close();
        //   exit;
    header('Location: request-a-trip.php');
    ?>